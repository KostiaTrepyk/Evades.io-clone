...
export class CommonSkill implements ISkill {
  ...
  private readonly _cooldown: CommonSkillOptions['cooldown'];
  private readonly _energyUsage: CommonSkillOptions['energyUsage'];
  private readonly _onUse: CommonSkillOptions['onUse'];
  private readonly _condition: CommonSkillOptions['condition'];
  private _lastUsedTimestamp: number | null;

  constructor(player: CharacterBase, options: CommonSkillOptions) {
  ...
  public isNotCooldown(): boolean {
    ...
    const isCooldown = time.timestamp < this._cooldown() + this._lastUsedTimestamp;
  ...
  public isEnoughEnergy(): boolean {
    return this._player.characteristics.energy.current - this._energyUsage() >= 0;
  }

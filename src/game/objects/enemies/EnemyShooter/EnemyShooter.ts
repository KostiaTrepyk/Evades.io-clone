...
export class EnemyShooter extends EnemyBase {
  ...
  /** Time in seconds */
  private readonly _shootDeltaTime: number;
  private readonly _projectileSpeed: number;
  private readonly _shootDistance: number;
  ...
  constructor(params: EnemyShooterParams) {
    ...
    super({
      position,
      radius: ENEMYSHOOTERCONFIG.radius,
      velocity,
  ...
  private createProjectile(params: {
    ...
    const projectile = new EnemyShooterProjectile({
      ...
      velocity: this.computeProjectileVelocity(params.playerPosition, this._projectileSpeed),
    });
  ...
  private computeProjectileVelocity(playerPosition: Position, speed: number): Velocity {

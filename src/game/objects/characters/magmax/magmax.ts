...
export class Magmax extends CharacterBase {
  ...
  constructor(startPosition: Position) {
    ...
    this.firstSkill = new ToggleSkill(this, {
      ...
      condition: () => this.level.upgrades.firstSpell.current > 0 && !this.isDead,
    });
    ...
    this.secondSkill = new ToggleSkill(this, {
      ...
      condition: () => this.level.upgrades.secondSpell.current > 0 && !this.isDead,
    });
  }
  ...
  private applySpeedBoost(): void {
    const speedBoost = this.spellsUpgrades.first.speed[this.level.upgrades.firstSpell.current - 1];
